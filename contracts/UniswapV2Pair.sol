...
contract UniswapV2Pair is IUniswapV2Pair {
    ...
    /**
     * @dev last cumulative price of token0
     **/
    uint256 public price0CumulativeLast;

    /**
     * @dev last cumulative price of token1
     **/
    ...
    function initialize(address _token0, address _token1) external {
        ...
        require(_token0 != address(0), 'UniswapV2: _token0 should not be the zero address');
        require(_token1 != address(0), 'UniswapV2: _token1 should not be the zero address');
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            liquidity = ((amount0 * _totalSupply) / _reserve0).min((amount1 * _totalSupply) / _reserve1);
        }
    ...
    /**
     * @param balance0 balance amount of token0
     * @param balance1 balance amount of token1
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint256 rootK = (uint256(_reserve0).sqrt() * _reserve1).sqrt();
